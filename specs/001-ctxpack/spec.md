...
- **FR-004**: System MUST score and rank files using keyword overlap scoring. The `--task` string is tokenized into lowercase words (length >= 3). Points are awarded for matches in path (3 points) and content (1 point, capped at 10 per token), normalized by file length and divided by `1 + depth` where depth is the number of path separators.
